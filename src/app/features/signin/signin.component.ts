...
@Component({
  selector: 'app-signin',
  imports: [MatFormFieldModule, MatInputModule, MatButtonModule, MatIconModule, MatTooltipModule, FormsModule, ReactiveFormsModule],
...
export class SigninComponent {
  ...
  readonly signinForm = new FormGroup({ emailOrUsername: new FormControl('', [Validators.required]), password: new FormControl('', [Validators.required]) })

  constructor(public authService: AuthService, public router: Router, activatedRoute: ActivatedRoute) {
    this.redirectTo = activatedRoute.snapshot.queryParamMap.get(REDIRECT_TO_PARAM_NAME);
  }
